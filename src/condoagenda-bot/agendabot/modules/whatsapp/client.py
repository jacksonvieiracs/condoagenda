...
class WhatsAppClient:
    ...
    async def send_plain_text_message(
        ...
        response = await self._client.post(
            f"/message/sendText/{self._current_istance}", json=data, timeout=2
        )
    ...
    async def get_connection_status(self) -> GetConnectionStatusResponse:
        response = await self._client.get(
            f"/instance/connectionState/{self._current_istance}"
        )
        data = response.json()
        status = (
    ...
    # methods to create a instance dynamic
    # methods to atributte dinamycs webhooks for example build a simple workflow
